import { BoundingBox, Vector3, Ray as MathRay } from '@galacean/engine-math';
import { BVHNode } from './BVHNode';
import { AABB } from './AABB';
import { Ray } from './Ray';
import { CollisionResult } from './CollisionResult';
import { BVHBuildStrategy } from './enums';
import type { BVHStats, BVHInsertObject } from './types';
...
export class BVHTree {
  ...
  private splitLeaf(leaf: BVHNode, newBounds: BoundingBox, newUserData: any, newObjectId: number): void {
    // 创建新叶子节点
    ...
    // 中点分割
    const mid = (existingData.bounds.min[splitAxis] + existingData.bounds.max[splitAxis] +
                 newBounds.min[splitAxis] + newBounds.max[splitAxis]) / 4;

    // 决定左右分配
    const existingMid = (existingData.bounds.min[splitAxis] + existingData.bounds.max[splitAxis]) / 2;
    const newMid = (newBounds.min[splitAxis] + newBounds.max[splitAxis]) / 2;

    const existingDataNode = BVHNode.createLeaf(existingData.bounds, existingData.userData, existingData.objectId, leaf.depth + 1);
  ...
  private insertRecursive(node: BVHNode, bounds: BoundingBox, userData: any, objectId: number, depth: number): void {
  ...
  private raycastRecursive(node: BVHNode, ray: Ray, results: CollisionResult[], maxDistance?: number): void {
    if (node.isLeaf) {
  ...
  private findNearestRecursive(node: BVHNode, position: Vector3, candidates: { distance: number; data: any }[], maxDistance?: number): void {
    if (node.isLeaf) {
