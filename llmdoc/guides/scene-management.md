---
id: "guide-scene-management"
type: "guide"
title: "场景管理指南"
description: "详细介绍场景创建、切换、实体管理和场景设置"
tags: ["guide", "scene", "entity", "scene-switching", "lifecycle"]
context_dependency: ["coding-conventions"]
related_ids: ["guide-component-system", "guide-asset-loading"]
---
...
### 关键约束
- **场景所有权**: 场景必须由 `SceneManager` 管理，不可手动创建独立的场景实例
- **实体归属**: 实体必须属于某个场景，不可创建悬空实体（无场景归属）
- **切换原子性**: 场景切换必须是原子操作，不允许在切换过程中同时使用新旧场景
- **根实体限制**: 场景只能有一个根实体层级，不可创建多个根实体直接挂在场景上

### 常见错误
- **场景泄漏**: 切换场景时未正确卸载旧场景，导致资源占用持续增长
- **实体引用残留**: 场景销毁后仍持有实体引用，访问时导致空指针异常
- **异步切换冲突**: 在异步加载新场景时直接切换，造成场景状态不一致
- **父子关系断裂**: 在场景切换时未正确维护实体的父子关系，导致渲染异常

### 最佳实践
- **场景预加载**: 在切换场景前异步预加载目标场景资源，减少切换时的卡顿
- **状态保存**: 切换场景前保存当前场景状态（如玩家位置、关卡进度）
- **分场景管理**: 大型世界使用分块场景管理，按需加载和卸载场景块
- **统一入口**: 使用统一的场景管理器处理所有场景切换逻辑，避免分散处理
- **错误回退**: 场景加载失败时回退到安全场景或显示错误提示，避免应用崩溃