---
id: "guide-asset-loading"
type: "guide"
title: "资源加载和管理指南"
description: "详细介绍资源管理器、异步加载、缓存机制和资源生命周期管理"
tags: ["guide", "asset-management", "resource-loader", "cache", "async-loading"]
context_dependency: ["coding-conventions"]
related_ids: ["guide-scene-management", "guide-performance-optimization"]
---
...
### 关键约束
- **引用计数精度**: 必须确保每个资源引用都有对应的释放调用，引用计数不可为负数
- **资源类型安全**: 加载资源时必须指定正确的泛型类型，避免运行时类型错误
- **路径格式统一**: 资源路径必须使用正斜杠 `/`，不可使用反斜杠或平台相关路径
- **异步处理完整性**: 所有异步加载必须包含错误处理，不可忽略 `Promise` 拒绝状态

### 常见错误
- **资源泄露**: 在场景切换或实体销毁时不释放不再使用的资源引用
- **重复加载**: 相同资源多次调用 `load` 方法而不检查缓存，造成内存浪费
- **类型不匹配**: 强制转换错误的资源类型，如将音频数据当作纹理加载
- **同步等待异步**: 在主线程中使用 `while` 循环等待异步加载完成，导致界面卡死

### 最佳实践
- **预加载策略**: 在场景加载前预加载关键资源，避免运行时卡顿
- **引用计数管理**: 使用 `try-finally` 块确保资源引用正确释放
- **缓存清理策略**: 实现 LRU 算法自动清理长时间未使用的资源
- **进度反馈**: 大型资源加载时提供进度回调，改善用户体验
- **降级处理**: 为关键资源提供降级方案，确保在网络异常时应用仍可运行