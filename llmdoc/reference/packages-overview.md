...
#### @galacean/bvh
- **功能职责**: BVH（包围体层次结构）加速结构，提供高效的空间查询和碰撞检测
- **主要API**:
  - `BVHTree` - BVH树主类，管理对象和查询操作
  - `BVHNode` - BVH树节点结构
  - `AABB`, `BoundingSphere` - 包围体实现
  - `BVHBuilder` - BVH构建器，支持多种构建策略
  - `Ray`, `CollisionResult` - 光线投射和碰撞结果
- **使用场景**: 碰撞检测、光线投射、空间查询、物理模拟、渲染优化
- **依赖关系**: 依赖 core, math
- **包大小**: ~35KB (压缩后)
- **复杂度**: 中等 (空间数据结构)
...
// 空间加速组合
import { Engine } from "@galacean/engine";
import { BVHTree, AABB } from "@galacean/bvh";
import "@galacean/physics-lite";
```

**场景**: 物理模拟、碰撞检测、射线追踪、空间查询
**特点**: 高性能空间索引、O(log n)查询复杂度
